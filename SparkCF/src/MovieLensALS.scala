...
object MovieLensALS {
  ...
  def main(args: Array[String]) {
    ...
    // 装载用户评分，该评分由评分器生成
    val myRatings = loadRatings("/home/hdanaly/xrli/sparkExamples/MovieCF/personalRatings.txt.template")
    val myRatingsRDD = sc.parallelize(myRatings, 1)
 
 
    // 装载样本评分数据，其中最后一列Timestamp取除10的余数作为key，Rating为值,即(Int,Rating)
    val ratings = sc.textFile("xrli/ratings.dat").map { line =>
      val fields = line.split("::")
      (fields(3).toLong % 10, Rating(fields(0).toInt, fields(1).toInt, fields(2).toDouble))
    }
    //这里的Rating是个case类，是ALS中定义的。
 
    // 装载电影目录对照表（电影ID->电影标题）
    val movies = sc.textFile("xrli/movies.dat").map { line =>
      val fields = line.split("::")
